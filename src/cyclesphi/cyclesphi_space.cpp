...
void bspace_loop(FromCLSpace &fromCL)
{
  while (true) {
    ...
    spaceConverterServerTcp.init_sockets_data(
        fromCL.space_server.c_str(), fromCL.space_server_port, false);
    ...
    while (true) {
      ...
      if (message_type == 1) {
        ...
        spaceConverterServerTcp.recv_data_data(
            (char *)particle_data_types.data(), sizeof(char) * s, false);
      ...
      if (message_type == 2) {
        ...
        spaceConverterServerTcp.send_data_data(
            (char *)&spaceData.bbox_min[0], sizeof(float) * 3, false);
        spaceConverterServerTcp.send_data_data(
            (char *)&spaceData.bbox_max[0], sizeof(float) * 3, false);
        spaceConverterServerTcp.send_data_data((char *)&spaceData.bbox_dim, sizeof(int), false);
        spaceConverterServerTcp.send_data_data(
            (char *)&spaceData.grid_transform, sizeof(float), false);
        spaceConverterServerTcp.send_data_data(
            (char *)&spaceData.particle_type, sizeof(int), false);
        spaceConverterServerTcp.send_data_data(
            (char *)&spaceData.block_name_id, sizeof(int), false);
        // spaceConverterServerTcp.send_data_data((char*)&spaceData.value_convert, sizeof(int),
        // false);
        spaceConverterServerTcp.send_data_data(
            (char *)&spaceData.extracted_type, sizeof(int), false);
        ...
        spaceConverterServerTcp.send_data_data(
            (char *)&spaceData.filter_max, sizeof(float), false);
        ...
        spaceConverterServerTcp.send_data_data(
            (char *)&spaceData.anim_task_counter, sizeof(int), false);
        ...
        spaceConverterServerTcp.recv_data_data(
            (char *)&spaceData.max_value_reduced, sizeof(spaceData.max_value_reduced), false);
        bSpaceClientTcp.send_data_data(
            (char *)&spaceData.max_value_reduced, sizeof(spaceData.max_value_reduced), false);
  ...
  while (true) {
    ...
    if (grid_handle_final.size() > 0) {
      ...
      if (volume_geom_name && volume_attr_name) {
        for (auto &op : options) {
          xml_set_volume_to_attr(
              op.scene, volume_geom_name, volume_attr_name, file_type, grid_handle_final);
        }
      }
    }

    // loader_data.clear();
    // render_loop(isHeadNode, loader_data, loader_radius, loader_lights, hanari, world, workers,
    // blenderClientTcp);
