// #####################################################################################################################
// # Copyright(C) 2011-2025 IT4Innovations National Supercomputing Center, VSB - Technical
...
#  define DEBUG_END_TIME(name) \
    double t2_##name = omp_get_wtime(); \
    printf("Elapsed time: %f, FPS: %f, %s\n", \
           t2_##name - t1_##name, \
           1.0 / (t2_##name - t1_##name), \
           #name);
#else
...
void session_init(FromCL &fromCL, Options &options, int session_id)
{
  ...
  ccl::vector<ccl::DeviceInfo> devices = ccl::Device::available_devices(
      (ccl::DeviceTypeMask)(1 << device_type));
  ...
  if (!devices.empty()) {
    options.session_params.device = ccl::Device::get_multi_device(
        devices, 0, false);  // devices.front();
  ...
  //	{
  //		//#else
  //		auto output_driver = std::make_unique<ccl::FrameOutputDriver>(options.output_pass,
  //session_print); 		options.output_driver = output_driver.get();
              ...
              std::vector<Options> &options)
{
  ...
  //
  // #ifdef WITH_CLIENT_GPUJPEG
  //	ccl::device_vector<ccl::half4> temp_buffer(options.session->device, "buffer",
  //ccl::MemoryType::MEM_DEVICE_ONLY); #endif
  ...
  while (fromCL.render_running) {
    ...
    if (blenderClientTcp->is_error()) {
      printf("Recv. renderengine_data: TCP Error!");
      break;
    }

    if (g_renderengine_data_rcv.magic_number != 999) {
      printf("Recv. renderengine_data: Wrong magic number!");
      break;
    }
    ...
    // check animation
    if (options.size() > 1 && blenderClientTcp->get_frame() >= 0 &&
        blenderClientTcp->get_frame() < options.size())
    {
    ...
    if (data_render_aux_rcv.data.size() > 0) {
      blenderClientTcp->recv_data_data((char *)data_render_aux_rcv.data.data(),
                                       data_render_aux_rcv.data.size());
    ...
    if (pixels_buf_empty.size() !=
        sizeof(ccl::half4) * g_renderengine_data_rcv.width * g_renderengine_data_rcv.height)
    {
      pixels_buf_empty.resize(sizeof(ccl::half4) * g_renderengine_data_rcv.width *
                              g_renderengine_data_rcv.height);
    }
    ...
    try {
      // cam_change
      if (/*renderer == NULL || */ memcmp(
          main_renderengine_data, &g_renderengine_data_rcv, sizeof(renderengine_data)))
      {
       ...
       // cuDeviceSet(0);
          ...
          //					//	main_options->display_driver->d_pixels =
          //(void*)temp_buffer.device_pointer; #endif
        ...
        ccl::Transform tfm = ccl::transform_clear_scale(
            ccl::make_transform(input[0],
                                input[1],
                                input[2],  // First row
                                input[3],
                                input[4],
                                ...
                                input[9],
                                input[10],
                                input[11]  // Fourth row (Translation vector)
                                ) *
      ...
      if (data_render_aux_rcv.data.size() > 0 && memcmp(main_data_render_aux->data.data(),
                                                        data_render_aux_rcv.data.data(),
                                                        data_render_aux_rcv.data.size()))
      {
        DEBUG_START_TIME(material);
        memcpy(main_data_render_aux->data.data(),
               data_render_aux_rcv.data.data(),
               data_render_aux_rcv.data.size());

        // memcpy(xf.colorMap.data(), cyclesphiDataRender.colorMap, sizeof(vec4f) *
        // xf.colorMap.size()); xf.domain = range1f(cyclesphiDataRender.domain[0],
        // cyclesphiDataRender.domain[1]); xf.baseDensity = cyclesphiDataRender.baseDensity;
        ...
        {
          // xml_set_volume_to_attr(main_options->scene, volume_geom_name, volume_attr_name,
          // file_type, grid_handle_final); xml_set_material_to_node(main_options->scene,
          // cyclesphiDataRender.data());
      ...
      // blenderClientTcp->save_bmp(main_options->width, main_options->height,
      // (char*)main_options->output_driver->pixels.data(), main_options->session_samples);
      // DEBUG_START_TIME(send_output);
      // blenderClientTcp->send_data_data((char*)main_options->output_driver->pixels.data(),
      // pixels_buf_empty.size()); DEBUG_END_TIME(send_output);

      if (main_options->display_driver) {
        DEBUG_START_TIME(send_gpujpeg_display);
        blenderClientTcp->send_data_data((char *)main_options->display_driver->pixels.data(),
                                         pixels_buf_empty.size());
        DEBUG_END_TIME(send_gpujpeg_display);
      }
